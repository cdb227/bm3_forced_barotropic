...
def integrate_ensemble(nlat,nlon, tstep, T, ofreq, ics, forcing_type='gaussian', n_ens = 5, nonlinear=False, vortpert=0., thetapert=0.):
    ...
    re-initiated with each ensemble member"""
    #TODO: right now this only wroks with our gaussian blob forcing
    st= Sphere(nlat,nlon)
    F = Forcing(st,tstep,T)
    if forcing_type=='gaussian':
        forcing_tseries=F.generate_gaussianblob_tseries()
    if forcing_type=='stochastic_eddy':
        forcing_tseries=F.generate_stocheddy_tseries(A=32e-10)
    for ee in range(n_ens):
        st= Sphere(st.nlat,st.nlon)
        
        ics_ve = ics[0] + np.random.normal(loc=0., scale = vortpert, size= ics[0].shape)
        ics_te = ics[1] + np.random.normal(loc=0., scale = thetapert)
        
        solver = Solver(st, tstep=tstep, T=T, ofreq=ofreq, forcing_tseries=forcing_tseries, ics = np.array([ics_ve,ics_te]))
        if ee==0:
            slns = solver.integrate_dynamics(nonlinear=nonlinear)
        else:
            slns = xr.concat([slns,solver.integrate_dynamics(nonlinear=nonlinear)], "ens_mem")
